# /tests/tools/conversation/test_memory_tools.py
...
def mock_dbs():
    ...
    # Giả lập đối tượng redis bên trong cache
...
async def test_store_user_preference_success(memory_tool, mock_dbs):
    ...
    mock_cache.redis.hset.return_value = 1 # Giả lập Redis trả về 1 khi set thành công
...
async def test_get_user_preferences_success(memory_tool, mock_dbs):
    ...
    # Dữ liệu giả lập trả về từ Redis (dưới dạng bytes)
    mock_redis_data = {
        b'favorite_topic': b'history',
...
async def test_get_user_preferences_empty(memory_tool, mock_dbs):
    ...
    # 1. ARRANGE
    mock_cache, _ = mock_dbs
    mock_cache.redis.hgetall.return_value = {} # Giả lập không có dữ liệu
